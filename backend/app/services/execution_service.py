...
class ExecutionService:
    ...
    @staticmethod
    async def _prepare_chat_context(db: AsyncSession, req: ExecutionRequest) -> Tuple[Any, List[ArtifactSchema]]:
        ...
        return db_session, pydantic_artifacts

    @staticmethod
    async def _process_generated_artifacts(db: AsyncSession, session_id: str, new_artifacts_map: Dict, req: ExecutionRequest) -> List[Artifact]:
        new_artifact_models = []
    ...
    async def execute_stream(db: AsyncSession, req: ExecutionRequest):
        db_session, pydantic_artifacts = await ExecutionService._prepare_chat_context(db, req)
        ...
        async for event in provider.generate_stream(llm_req):
            ...
            if isinstance(event, TextDeltaEvent):
                ...
                payload = {"type": "text_delta", "content": event.content}
                ...
                payload = {"type": "artifact_end", "artifact_id": event.artifact_id}

            if payload:
                yield f"data: {json.dumps(payload)}\n\n"

        # Process generated artifacts and save to DB
        new_artifact_models = await ExecutionService._process_generated_artifacts(db, db_session.id, new_artifacts_map, req)
