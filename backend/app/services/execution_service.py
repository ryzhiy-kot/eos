...
class ExecutionService:
    ...
    @staticmethod
    async def _prepare_chat_context(db: AsyncSession, req: ExecutionRequest) -> tuple[Any, LLMRequest]:
    ...
    async def _finalize_chat_execution(
        ...
        assistant_msg = await SessionService.save_message(
            db, session_id, "assistant", full_text, artifacts=new_artifact_models
        )
        return assistant_msg

    @staticmethod
    async def chat_stream(db: AsyncSession, req: ExecutionRequest) -> AsyncGenerator[str, None]:
        db_session, llm_req = await ExecutionService._prepare_chat_context(db, req)
        ...
        await ExecutionService._finalize_chat_execution(
            db, req.session_id, db_session.id, full_text, new_artifacts_map, req.action or ""
        )
    ...
    async def _execute_chat(db: AsyncSession, req: ExecutionRequest) -> Dict:
        ...
        assistant_msg = await ExecutionService._finalize_chat_execution(
            db, req.session_id, db_session.id, full_text, new_artifacts_map, req.action or ""
        )
