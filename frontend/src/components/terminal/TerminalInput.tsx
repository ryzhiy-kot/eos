...
import { PaneType, ResourceType } from '@/types/constants';
...
const TerminalInput: React.FC<TerminalInputProps> = ({
    ...
    const attachments = useMemo(() => {
        ...
        return Array.from(new Set(refs)).map(ref => {
            ...
            let label = ref;
            let type: ResourceType = ResourceType.ARTIFACT;

            if (id.startsWith('P')) {
                type = ResourceType.PANE;
                const p = panes[id];
            ...
            {attachments.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-1 animate-in fade-in slide-in-from-bottom-1 duration-200">
                    {attachments.map((at, i) => (
                        ...
                        >
                            <span className="opacity-70">{at.type === ResourceType.PANE ? 'Pane' : 'File'}:</span>
            ...
            <div className="flex items-center gap-3">
                ...
                <form onSubmit={onSubmit} className="flex-1 relative flex items-center overflow-hidden">
                    ...
                    <input
                        ...
                        placeholder={paneType === PaneType.CHAT ? "Type a message..." : "Enter command..."}
                        autoFocus
                    />
                </form>
            </div>
